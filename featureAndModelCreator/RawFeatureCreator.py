...
def extract_features_for_exe(exe_folder, is_malware, exe_net_path):
    ...
    feature_dict['Unique_Hash_Count'] = len(hashes.keys()) / total_events
    ...
    feature_dict['Desc_Entropy'] = description_entropy_value / total_events
    feature_dict['Product_Entropy'] = product_entropy_value / total_events
    feature_dict['Company_Entropy'] = company_entropy_value / total_events
    ...
    feature_dict['Integrity_Ratio'] = (high_privilege_integrity_count) / (total_events)
    ...
    feature_dict['Cmd_Flag_Extension'] = command_line_extension_count / total_events
    feature_dict['Cmd_Flag_Tokens'] = command_line_num_tokens / total_events
    ...
    feature_dict['Cmd_Num_Length'] = command_line_cmd_length / total_events
    ...
    feature_dict['Unique_Operational_Id_Ratio'] = len(set(operationIds)) / (total_events)

    feature_dict['Rule_Names_Entropy'] = rule_names_entropy_value

    feature_dict['Avg_File_Count'] = fileCount_count / total_events

    feature_dict['File_Version_Diff_Scaled'] = (file_version_max - file_version_min) / total_events

    feature_dict['File_Version_Non_Numeric_Weighted'] = file_version_non_numeric / total_events
...
def calculate_statistical_features(event_counts, event_times, file_paths, process_ids, parent_processes,
    ...
    feature_dict['Event_Kurtosis_Skewness'] = (skew(event_count_values) + 1)
...
combined_matrix.to_csv('Raw_Feature_Set.csv', index=False, float_format='%.12f')
