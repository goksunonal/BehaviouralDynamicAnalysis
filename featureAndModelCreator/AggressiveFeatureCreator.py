...
def extract_features_for_exe(exe_folder, is_malware, exe_net_path):
    ...
    feature_dict['Adjusted_Service_Entropy_Avg_Proportion'] = service_proportion / (
            total_events + np.var(service_entropy_value))

    feature_dict['Auth_Event_Ratio'] = authentication_event_count / total_events
    feature_dict['Privilege_Subject_Event_Ratio'] = (privilege_event_count / total_events) * (
            subject_event_count / total_events)
    ...
    feature_dict['Parent_Process_Ratio_x_Hash_Count'] = (len(hashes.keys()) + 1) * (
            parent_processes_ratio + 1) / total_events * ((np.std(
        [parent_processes_ratio + 1, len(set(hashes.keys())) + 1])) + 1)
    ...
    feature_dict['Original_File_Ext'] = original_file_ext * 2 / (total_events * (original_file_length + 1))
    feature_dict['Original_File_Lower'] = original_file_lower_ratio / (total_events * (original_file_length + 1))

    feature_dict['Adjusted_Original_File_Alpha_Numeric'] = (original_file_alpha_numeric * 0.8) / (
            total_events + original_file_entropy)
    ...
    feature_dict['Adjusted_Unique_Process_Guid_Utc_Ratio'] = (len(set(process_guid_count)) * utc_times) / (
            total_events * np.std([len(set(process_guid_count)), utc_times]) + 1)

    feature_dict['Desc_Product_Company_Entropy_Proportion'] = (product_entropy_value + company_entropy_value) / (
            total_events * (description_entropy_value * 2 + 1))
    ...
    feature_dict['Adjusted_Desc_Product_Company_Avg_Proportion'] = ((product_proportion + 1) * (
            company_proportion + 1)) / (total_events * (np.mean(
        [product_proportion, company_proportion, description_avg_proportion]) + 1))
    ...
    feature_dict['Avg_Metadata_Path_Entropy'] = ((metadata_entropy + 1) * (metadata_path + 1)) / (total_events * 2)
    feature_dict['Avg_Metadata_Digit_Char'] = ((metadata_digit + 1) * (metadata_char + 1)) / (total_events * 2)
    ...
    feature_dict['Adjusted_High_Privilege_Integrity_Ratio'] = high_privilege_integrity_count / (
            total_events * (np.sum(privilege_event_count) + 1))
    ...
    feature_dict['Adjusted_Cmd_Extension_Count'] = command_line_extension_count / (
            1 + np.std([command_line_num_tokens, command_line_extension_count])) * total_events

    feature_dict['Cmd_Encoded_Cmd'] = command_line_encoded_command / total_events
    feature_dict['Cmd_Obf_Flag'] = command_line_obfuscation_flags / total_events

    feature_dict['Cmd_Dir_Combined'] = (command_line_temp_directory + command_line_appdata_directory) / 2 * total_events

    feature_dict['Cmd_Num_Uppercase'] = command_line_num_uppercase / total_events
    feature_dict['Cmd_Length_x_Num_Digits'] = command_line_cmd_length / (total_events * (command_line_num_digits + 1))
    feature_dict['Cmd_Num_Special_Chars'] = command_line_num_special_chars / total_events

    feature_dict['Source_Avg_Entropy_Proportion'] = (composite_text_complexity_score(source_normalized_word_count,
                                                                                     source_digit_proportion,
                                                                                     source_capital_proportion,
                                                                                     source_symbol_proportion,
                                                                                     source_dictionary_word_proportion,
                                                                                     source_random_pattern_flag) /
                                                     (total_events * (source_entropy_value + 1)))

    feature_dict['Unique_Message_Line_Numbers'] = (len(set(unique_line_number)) + 1) * (len(set(message_count)) + 1) / (
            total_events * np.mean([len(unique_line_number) + 1, len(message_count) + 1]))

    feature_dict['Unique_Opcode_Ratio'] = len(set(opcodes)) / total_events

    feature_dict['Opcode_Entropy'] = entropy(pd.DataFrame(opcodes).value_counts(normalize=True))

    feature_dict['Unique_Keyword_Task_Ratio'] = (len(set(keywords)) + 1) * (len(
        set(tasks)) + 1) * (len(set(unique_thread_id)) + 1) / (total_events *
                                                               np.mean([len(set(keywords)) + 1, len(set(tasks)) + 1,
                                                                        len(set(unique_thread_id)) + 1]))

    feature_dict['Event_Task_Keyword_Entropy'] = (entropy(list(event_counts.values())) * 0.2 + entropy(
        pd.DataFrame(set(tasks)).value_counts(normalize=True)) * 0.3 + entropy(
        pd.DataFrame(set(keywords)).value_counts(normalize=True)) * 0.5)

    feature_dict['Unique_System_Provider_HC_State_Ratio'] = len(set(hc_state_count)) / (
            total_events + len(set(system_provider_names)))

    feature_dict['System_Provider_Entropy'] = entropy(
        pd.DataFrame(set(system_provider_names)).value_counts(normalize=True))
    feature_dict['Unique_Operational_Id_Ratio'] = len(set(operationIds)) / (total_events + len(set(operationIds)))

    feature_dict['Rule_Names_Entropy_Ratio'] = composite_text_complexity_score(rule_names_normalized_word_count,
                                                                               rule_names_digit_proportion,
                                                                               rule_names_capital_proportion,
                                                                               rule_names_symbol_proportion,
                                                                               rule_names_dictionary_word_proportion,
                                                                               rule_names_random_pattern_flag) / (
                                                       total_events + rule_names_entropy_value)

    feature_dict['Avg_File_Count'] = fileCount_count / total_events

    feature_dict['Aggregated_File_Version_Feature'] = (
                                                              file_version_max - file_version_min) * 0.2 + file_version_non_numeric * 1.6 + (
                                                          file_version_build_info) * 1.6 / total_events * (np.mean(
        [file_version_length, file_version_non_numeric, file_version_build_info]) + 1)
...
def calculate_statistical_features(event_counts, event_times, file_paths, process_ids, parent_processes,
    ...
    feature_dict['Mean_Median_Event_Count'] = ((pd.Series(event_count_values).mean() + 1) * (
        pd.Series(event_count_values).median()) + 1) / total_events

    feature_dict['Event_Kurtosis_Skewness'] = (kurtosis(event_count_values) + 1) * (skew(event_count_values) + 1)
...
combined_matrix.to_csv('Aggressive_Feature_Set.csv', index=False, float_format='%.12f')
