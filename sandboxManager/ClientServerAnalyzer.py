...
def terminate_process_by_pid(pid):
    try:
        ...
        print('New packet, Process found dead, no need to kill')
...
def analyze_folder(url, dir_path, timeout, logcat_text, fullAnalyze):
    TcpClient.set_base_url(url)
    filename = TcpClient.get_next_file()
    if filename is None:
        return
    ...
    try:
        ...
        if filename.endswith('.exe'):
            ...
            process = subprocess.Popen(['cmd.exe', '/c', full_file], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        elif filename.endswith('.ps1'):
            process = subprocess.Popen(['powershell', '-ExecutionPolicy', 'Bypass', '-File', full_file],
                                       stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        elif filename.endswith('.jar'):
            process = subprocess.Popen(['java', '-jar', full_file], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        else:
    ...
    try:
        ...
        TcpClient.upload_log_folder(filename, directory_name)
    except Exception as e:
        print(f'An error occurred: {str(e)}')
...
def start_process():
    ...
    exe_thread = threading.Thread(target=analyze_folder,
                                  args=(url, selected_directory, int(timeout), logcat_text, False))
    exe_thread.start()


def start_process_from_arguments():
    selected_directory = sys.argv[2]
    timeout = sys.argv[4]
    url = sys.argv[3]
    directory_entry.insert(tk.END, selected_directory)
    timeout_entry.insert(tk.END, timeout)
...
def start_process_full():
    ...
    logcat_text.yview(tk.END)
    exe_thread = threading.Thread(target=analyze_folder,
                                  args=(url, selected_directory, int(timeout), logcat_text, True))
    exe_thread.start()
...
url_label = tk.Label(root, text='Upload Server Ip')
url_label.pack()
...
if len(sys.argv) > 3:
    ...
    sys_thread = threading.Thread(target=start_process_from_arguments, args=[])
    sys_thread.start()
else:
